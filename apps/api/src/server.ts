...
app.post("/auth/request-otp", async (request, reply) => {
  ...
  if (!isEmail && !/^\+?[1-9]\d{7,14}$/.test(contact)) return reply.code(400).send({ error: "Enter a phone number with country code" });
...
app.post("/auth/register", async (request, reply) => {
  ...
  if (!body.success) return reply.code(400).send({ error: "Check the registration details" });
...
app.post("/auth/sign-in", async (request, reply) => {
  ...
  if (!/^[a-z0-9_]{3,24}$/.test(username)) return reply.code(401).send({ error: "Incorrect username or password" });
...
app.post("/identity/aadhaar-dev", { preHandler: requireMember }, async (request, reply) => {
  ...
  if (!body.success) return reply.code(400).send({ error: "Invalid Aadhaar development reference" });
  ...
  while (queue.length && visited.size < 100) {
    ...
    for (const link of links) {
      ...
      if (link.relativeUserId) {
        ...
        if (link.relativeUser) users.set(link.relativeUser.id, memberSummary(link.relativeUser));
...
app.put("/rishte/me", { preHandler: requireMember }, async (request, reply) => {
  ...
  if (!body.success) return reply.code(400).send({ error: "Invalid Rishte profile" });
...
app.post("/rishte/:userId/interest", { preHandler: requireMember }, async (request, reply) => {
  ...
  if (!target?.isActive || target.user.suspendedAt || await blocked(request.user.userId, params.data.userId)) return reply.code(404).send({ error: "Rishte profile unavailable" });
...
app.post("/messages/threads/:id/messages", { preHandler: requireMember }, async (request, reply) => {
  ...
  if (!params.success || !body.success) return reply.code(400).send({ error: "Invalid message" });
