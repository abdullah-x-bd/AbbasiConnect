...
export function extractLikelyName(ocrText) {
    const lines = ocrText
        .split(/\r?\n/)
    ...
    if (dobIndex > 0) {
        for (let i = dobIndex - 1; i >= Math.max(0, dobIndex - 4); i -= 1) {
            if (looksLikeName(lines[i]))
                return titleCase(lines[i].replace(/[^A-Za-z .'-]/g, " ").replace(/\s+/g, " ").trim());
        }
    }
    for (const line of lines) {
        if (looksLikeName(line))
            return titleCase(line.replace(/[^A-Za-z .'-]/g, " ").replace(/\s+/g, " ").trim());
    }
    return "";
}
export async function scanAadhaarImage(imageDataUrl) {
