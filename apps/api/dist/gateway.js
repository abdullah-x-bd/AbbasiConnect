...
function eventDomain(method, url) {
    const verb = (method ?? "GET").toUpperCase();
    if (["GET", "HEAD", "OPTIONS"].includes(verb))
        return null;
    const path = (url ?? "").split("?")[0];
    if (path.startsWith("/messages-secure/") || path === "/crypto/me")
        return "messages";
    if (path.startsWith("/community/"))
        return "community";
    if (path.startsWith("/family/"))
        return "family";
    if (path.startsWith("/rishte/"))
        return "rishte";
    if (path === "/auth/me" || path.startsWith("/identity/"))
        return "profile";
    return null;
}
...
function internalJson(path, authorization) {
...
async function memberSession(authorization) {
    if (!authorization?.startsWith("Bearer "))
        return null;
    const result = await internalJson("/auth/session", authorization);
    if (!result.ok || result.data?.mode !== "member" || !result.data?.user?.id)
        return null;
    return result.data;
}
...
async function openPerformanceBootstrap(request, response) {
    if (request.method === "OPTIONS") {
        response.writeHead(204, corsHeaders());
        response.end();
        return;
    ...
    const entries = await Promise.all(Object.entries(paths).map(async ([key, path]) => [key, await internalJson(path, authorization)]));
...
function proxy(request, response) {
    ...
    const upstream = http.request({
        ...
        upstreamResponse.on("end", () => {
            ...
            if (domain && status >= 200 && status < 400)
                broadcast(domain);
        });
    });
    upstream.on("error", () => {
