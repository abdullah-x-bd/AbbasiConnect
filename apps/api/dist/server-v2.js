...
function ageFromDate(date) {
    ...
    if (md < 0 || (md === 0 && today.getUTCDate() < date.getUTCDate()))
        age -= 1;
    return age;
}
...
async function familyComponentIds(startId) {
    ...
    while (queue.length && visited.size < 150) {
        ...
        for (const link of links) {
            ...
            if (link.relativeUserId && !visited.has(link.relativeUserId))
...
// ---------- Authentication ----------
async function requestDevelopmentOtp(request, reply) {
    ...
    if (!isEmail && !/^\+?[1-9]\d{7,14}$/.test(contact))
        return reply.code(400).send({ error: "Use the full phone number with country code, for example +91…" });
...
app.post("/auth/register", async (request, reply) => {
    ...
    if (!body.success)
        return reply.code(400).send({ error: body.error.issues[0]?.message || "Check the registration details" });
    const contact = normalizeContact(body.data.contact);
...
app.post("/identity/aadhaar-dev", { preHandler: requireMember }, async (request, reply) => {
    ...
    if (!body.success)
        return reply.code(400).send({ error: "Invalid identity reference" });
...
app.get("/family/search", { preHandler: requireMember }, async (request, reply) => {
    ...
    if (!query.success)
        return reply.code(400).send({ error: "Enter at least two characters" });
...
app.post("/family/access/:targetId", { preHandler: requireMember }, async (request, reply) => {
    ...
    if ((await familyComponentIds(request.user.userId)).includes(params.data.targetId))
        return reply.code(409).send({ error: "This person is already in your connected family" });
...
app.get("/family/tree/:userId", { preHandler: requireMember }, async (request, reply) => {
    ...
    while (queue.length && visited.size < 100) {
        ...
        for (const link of links) {
            ...
            else if (!guests.some((guest) => guest.id === guestId))
                guests.push({ id: guestId, displayName: link.relativeName, registered: false });
        }
    }
...
app.put("/rishte/me", { preHandler: requireMember }, async (request, reply) => {
    ...
    if (!body.success)
        return reply.code(400).send({ error: "Invalid Rishte listing" });
...
app.post("/rishte/:userId/interest", { preHandler: requireMember }, async (request, reply) => {
    ...
    if (!target?.isActive || target.user.suspendedAt || await blocked(request.user.userId, params.data.userId))
        return reply.code(404).send({ error: "Rishte listing unavailable" });
...
app.delete("/community/comments/:id", { preHandler: requireMember }, async (request, reply) => {
    ...
    if (!comment || comment.authorId !== request.user.userId)
        return reply.code(403).send({ error: "Not allowed" });
...
app.put("/crypto/me", { preHandler: requireMember }, async (request, reply) => {
    ...
    if (!body.success)
        return reply.code(400).send({ error: "Invalid encryption key bundle" });
...
app.get("/crypto/users/:id/public-key", { preHandler: requireMember }, async (request, reply) => {
    ...
    if (!user.e2eePublicKey)
        return reply.code(409).send({ error: "This member has not enabled secure messaging yet" });
...
app.post("/messages-secure/threads/:id/messages", { preHandler: requireMember }, async (request, reply) => {
    ...
    if (!params.success || !body.success)
        return reply.code(400).send({ error: "Invalid encrypted message" });
    ...
    if (!other.e2eePublicKey || !me.e2eePublicKey)
        return reply.code(409).send({ error: "Both members must enable secure messaging before sending" });
